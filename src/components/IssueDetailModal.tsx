...
export function IssueDetailModal({ issueId, onClose }: IssueDetailModalProps) {
  ...
  if (!issue) {
    return (
      <Modal
        ...
        className="sm:max-h-[90vh] overflow-y-auto"
      >
        {/* biome-ignore lint/a11y/useSemanticElements: role="status" is correct for loading state */}
        <div role="status" aria-busy="true" className="p-4 sm:p-6 space-y-6">
  ...
  // Custom header with issue type icon, key, and priority badge
  const issueHeader = (
