...
function getConvexSiteUrl(): string {
  ...
  if (convexUrl.includes("127.0.0.1") || convexUrl.includes("localhost")) {
    // If explicitly provided 3211, keep it.
    // If provided 3210 (Client), assume HTTP Actions are on 3211.
    // If port 3211 is unreachable, we might need to fallback, but typically
    // `npx convex dev` uses 3210 for WS/Client and 3211 for HTTP Actions.
