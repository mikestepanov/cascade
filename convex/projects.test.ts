...
describe("Projects", () => {
  describe("create", () => {
    it("should create a project with default workflow states", async () => {
      ...
      const { companyId, workspaceId, teamId } = await createCompanyAdmin(t, userId);
      ...
      const projectId = await asUser.mutation(api.projects.createProject, {
        ...
        isPublic: false,
        boardType: "kanban",
        companyId,`n        workspaceId,`n        teamId,
      });
  ...
  describe("list", () => {
    it("should return only projects user is a member of", async () => {
      ...
      const { companyId: companyId1, workspaceId: workspaceId1, teamId: teamId1 } = await createCompanyAdmin(t, user1);
      const { companyId: companyId2, workspaceId: workspaceId2, teamId: teamId2 } = await createCompanyAdmin(t, user2);
  ...
  describe("updateWorkflow", () => {
    it("should allow admins to update workflow states", async () => {
      ...
      const { companyId } = const { companyId, workspaceId, teamId } = await createCompanyAdmin(t, adminId);
