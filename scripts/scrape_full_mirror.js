...
if (!targetUrl || !competitor || !pageName) {
  ...
  console.error('Example: node scripts/scrape_full_mirror.js https://linear.app/features linear features');
  process.exit(1);
}
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, `${pageName}_deep.json`), JSON.stringify(deepData, null, 2));
  console.log(`✅ Saved ${pageName}_deep.json (${Object.keys(deepData.cssVars).length} CSS vars, ${deepData.keyframes.length} keyframes)`);
  ...
  for (const entry of networkLog) {
    ...
    try {
      ...
      const filename = path.basename(urlObj.pathname) || `file_${downloaded}${getExtension(entry.url)}`;
